...
export const getVariant = (slug: string): IconVariant => {
...
export const removeVariant = (slug: string, variant: IconVariant) =>
  variant === "other" ? slug : slug.replace(`-${variant}`, "");

export const toComponentName = (fileName: string) =>
  fileName
    .split("-")
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1))
    .join("");

export const toDisplayName = (slug: string) =>
  slug
...
export const matchesSearch = (searchText: string, query: string) => {
  if (!query) {
    return true;
  }
...
export const getCategoryId = (baseSlug: string): IconCategoryId => {
