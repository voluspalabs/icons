...
const SVG_BLOCK_PATTERN = /<svg[\s\S]*?<\/svg>/u;
const PROPS_SPREAD_PATTERN = /\{\s*\.\.\.props\s*\}/gu;
const TAG_GAP_PATTERN = />\s+</gu;
const WHITESPACE_RUN_PATTERN = /\s+/gu;
const SPACE_BEFORE_CLOSE_PATTERN = /\s+>/gu;

// JSX uses camelCase SVG attributes; raw SVG markup needs the hyphenated forms.
const ATTRIBUTE_RENAMES: [RegExp, string][] = [
  [/\bstrokeWidth=/gu, "stroke-width="],
  [/\bstrokeLinecap=/gu, "stroke-linecap="],
  [/\bstrokeLinejoin=/gu, "stroke-linejoin="],
  [/\bstrokeMiterlimit=/gu, "stroke-miterlimit="],
  [/\bstrokeDasharray=/gu, "stroke-dasharray="],
  [/\bstrokeDashoffset=/gu, "stroke-dashoffset="],
  [/\bstrokeOpacity=/gu, "stroke-opacity="],
  [/\bfillRule=/gu, "fill-rule="],
  [/\bclipRule=/gu, "clip-rule="],
  [/\bfillOpacity=/gu, "fill-opacity="],
  [/\bclipPath=/gu, "clip-path="],
];
...
const run = async () => {
  ...
  for (const fileName of fileNames) {
    ...
    const baseName = baseSlug.replace(/^pi-/u, "");
    const categoryId = getCategoryId(baseSlug);

    // Sequential by design: reading 4k+ icon files with unbounded concurrency
  ...
  for (let index = 0; index < svgEntries.length; index += WRITE_CONCURRENCY) {
    ...
    // The loop is the concurrency limiter: each iteration writes one
    // WRITE_CONCURRENCY-sized batch.
    // oxlint-disable-next-line no-await-in-loop
    await Promise.all(
  ...
  const icons = [...families.values()]
    .toSorted((left, right) => left.baseName.localeCompare(right.baseName))
    .map((family) => ({
      c: family.categoryId,
      d: family.displayName,
      n: family.baseName,
      v: family.variants.toSorted(
        (left, right) =>
  ...
  await writeFile(
    ...
    buildLlmsTxt({ families: icons.length, siteUrl, total: manifest.total })
  );
